...
class OrderRepository(OrderRepositoryInterface):
    def get_by_id(self, customer_id: int, order_id: int):
        try:
            return self.convert_to_entity(Order.objects.get(customer=customer_id, id=order_id))
        except ObjectDoesNotExist:
            return None

    def get_pending_order(self, customer_id: int):
        try:
            return self.convert_to_entity(Order.objects.get(customer=customer_id, status=Order.PENDING))
        except ObjectDoesNotExist:
            return None
    ...
    def update_order(self, customer_id: int, order: OrderEntity) -> None:
        ...
        order_model.save()
        return self.convert_to_entity(order_model)

    def create_order(self, customer_id: int, order_data: OrderEntity | None) -> None:
        if order_data:
            self.convert_to_entity(Order.objects.create(customer=customer_id))
        else:
            return self.convert_to_entity(Order.objects.create(customer_id=customer_id))

    def delete_pending_order(self, customer_id):
        Order.objects.filter(customer=customer_id,
                             status=Order.PENDING).delete()

    def convert_to_entity(self, order: Order):
